...
def formatting(csvname):
    ...
    df = pd.read_csv('data/NAV/'+csvname, parse_dates=['date'], date_parser=dateparse, encoding='utf_8_sig')
...
def call_the_best(thelist):
    ...
    for i in range(len(thelist)):
        ...
        output += namedict[cat]+": "+str(name)+" 最佳，風險Q值為"+ sci_notation(q,4) +"\n"
    return output

def call_the_best_p(thelist, theindbest_list):
    namedictionary = {'2': "兩支基金組合", '3': "三支基金組合", '4':"四支基金組合", '5':"五支基金組合"}
    output = ""
    for i in range(len(thelist)):
        tempdf = thelist[i]
        name = tempdf.loc[tempdf["Ranking"]==1,"Name"].values[0]
        ...
        for k in range(len(nlist)):
            if k==0:
                portname = name_reconcile(theindbest_list[int(k)])[1]
            elif k==max(range(len(nlist))):
                portname = portname + "和" + name_reconcile(theindbest_list[int(k)])[1]
            else:
                portname = portname + "、" + name_reconcile(theindbest_list[int(k)])[1]
            
        q = tempdf.loc[tempdf["Ranking"]==1,"Q"].values[0]
        cat = tempdf.loc[tempdf["Ranking"]==1,"Category"].values[0]
        
        # print(namedict[cat]+": "+str(name)+" 最佳，風險Q值為"+str(q))
        print (namedictionary[cat]+"中，以 "+str(portname)+" 的搭配為最佳，組合風險Q值為"+ sci_notation(q,4))
    return output

def call_the_best_ps(nof, thelist, theindbest_list):
    namedictionary = {'2': "兩支基金組合", '3': "三支基金組合", '4':"四支基金組合", '5':"五支基金組合"}
    i = int(nof-2)
    tempdf = thelist[i]
    name = tempdf.loc[tempdf["Ranking"]==1,"Name"].values[0]
    ...
    # print(namedict[cat]+": "+str(name)+" 最佳，風險Q值為"+str(q))
    output = namedictionary[cat]+"中，以 "+str(portname)+" 的搭配為最佳，組合風險Q值為"+ sci_notation(q,4)
    return output
...
def name_reconcile(tejname):
    ...
    infoname = df.loc[df['tejname']==tejname,'moneydjname'].values[0]
...
def change_name_forq(theqlist):
    emp_list = []
    for i in theqlist:
        slist = []
        for k in range(i.shape[0]):
            slist.append(name_reconcile(i.iloc[k,1](1)))
